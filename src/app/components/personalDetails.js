...
export default function PersonDetails({ record, onBack }) {
  ...
  const finalAnalysis = record.fields.finalAnalysis
    ? JSON.parse(record.fields.finalAnalysis)
    : null;
        ...
        <div className="bg-gray-50 p-6 rounded-xl">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-900">{record.fields.email}</p>
            </div>
            <div>
            ...
            <div>
              <h3 className="text-sm font-medium text-gray-500">Task Relevancy</h3>
              <span
                className={`mt-1 px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                  finalAnalysis && finalAnalysis.isRelevantTask
                    ? "bg-green-100 text-green-800"
                    : "bg-red-100 text-red-800"
                }`}
              >
                {finalAnalysis && finalAnalysis.isRelevantTask ? "Relevant Task" : "Irrelevant Task"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 bg-indigo-50 p-6 rounded-xl border border-indigo-100">
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="bg-white p-5 rounded-lg shadow-sm">
              <h3 className="text-md font-medium text-gray-500 mb-2">Summary</h3>
              <p className="text-sm text-gray-800 leading-relaxed">{finalAnalysis.overallReport}</p>
            </div>
            {/* Stats Cards */}
      ...
      <div className="mt-8 space-y-6">
        ...
        <div className="bg-gray-50 p-6 rounded-xl">
          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200 max-h-60 overflow-y-auto">
            <p className="text-sm text-gray-700 whitespace-pre-wrap">{record.fields.systemPrompt}</p>
          </div>
        </div>
